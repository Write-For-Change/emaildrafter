...
ToDo:
    ...
    2. Edit the functions that use the returned email templates to get information correctly (i.e. e.subject is fine, but now target email is e.target["email"])
...
class EmailTemplate:
    ...
    def fill(self, user_info):
        ...
        if self.filled:
            print("Template Already Filled")
            return self.filled
        if self.target is None:
            print("Cannot fill template as target is empty")
            self.filled = False
            return self.filled

        self.body.format(t=self.target, u=user_info)
...
def get_existing_templates():
    print("Grab all the template options that exist so far")
