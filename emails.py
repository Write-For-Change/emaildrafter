...
def MPpoliceEmail(name, toname, ward, mpEmail):
    ...
    emailstring = """Dear {},<br><br>
        ...
        <br><br>
        {}
        """.format(toname, name, ward, name)

    return (mpEmail, subjectstring, None, emailstring)   
...
def gavinwilliamsonEmail(name, mpEmail):
    ...
    return ("gavin.williamson.mp@parliament.uk", subjectstring, mpEmail, emailstring)
...
def MPbellymujingaEmail(name, toname, ward, mpEmail):
    ...
    My name is {} and I am a resident of {} ward.
        <br><br>
...
def goviaBellymujingaEmail(name):
    ...
    return ("Patrick.Verwer@gtrailway.com", subjectstring, None, emailstring)
...
def getGovDetails(postcode):
    ...
    MPurl = "http://lda.data.parliament.uk/commonsmembers.json?_view=members&_pageSize=2097&_page=0&constituency.label="+"%20".join(topdata["parliamentary_constituency"].split(" "))
    ...
    with urllib.request.urlopen(MPurl) as url:
        ...
        for possibleMP in MPdata["result"]["items"]:
            ...
            try:
                ...
                print("Found correct MP: {}. {} emails found".format(MPname, len(MPemails))) 
                break
            except:
                pass
...
def draftEmails(myname, postcode):
    ...
    print("Details found. You live in {} ward and your MP is {}, with email(s): {}".format(ward, MPname, MPemails))

    emailTups = []

    for MPemail in MPemails:
        emailTups.append(MPpoliceEmail(myname, MPname, ward, MPemail))
        emailTups.append(MPbellymujingaEmail(myname, MPname, ward, MPemail))
    
    emailTups.append(gavinwilliamsonEmail(myname, MPemails[0]))
    emailTups.append(goviaBellymujingaEmail(myname))

    for tup in emailTups:
        Emailer(tup)    
    

myname = input("Please enter your full name (case sensitive): ")
mypost = input("Please enter your postcode (this will only be used to find your MP's email address): ")

draftEmails(myname, mypost)

print("There should now be emails drafted in your Outlook app. Please check over them and send them at your own discretion")
print("If you had any issues with this application, would like to add email templates to it, or want to help improve it, please contact Puria at pr450@cam.ac.uk") 
